...
<div id="page-wrapper" style="min-height: 172px;">
			<div class="main-page">
				<div class="tables">
					...
					<h2 class="title1">الاقسام</h2>



					<div class="bs-example widget-shadow" data-example-id="contextual-table">
            <h4 style="display:inline-block">الاقسام :</h4>
<?php
            ...
            if(check_permission($admin->group_id,$permission_create->id)){
             ?>
            <a class="btn btn-default" href="<?=$location;?>/views/admin/category/create.php" style="background-color: #F2B33F;float: left;display: inline-block;">
  ...
  <?php } ?>
                ...
                <thead>
                  ...
                  <tbody>
                    <?php $i=1; foreach ($allCategories as $key) {?>
                      <tr>
                        <td><?=$i;?></td>
                        <td><?=$key->name;?></td>
                        ...
                        <td>
                              <?php
                           $permission_edit= Permission::where('name','الاقسام')->where('action','update')->first();
                          if(check_permission($admin->group_id,$permission_edit->id)){
                           ?>
                           <a href="<?=$location?>/views/admin/category/edit.php?id=<?=$key->id;?>">
                        	<i class="fa fa-fw fa-edit"></i></td>
                           </a>
                           <?php }?>
                        <td>
                            <?php
                           $permission_delete = Permission::where('name','الاقسام')->where('action','delete')->first();
