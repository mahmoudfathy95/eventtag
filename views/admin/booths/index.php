...
if (isset($_GET['id'])) {
  $id=$_GET['id'];
  $allbooths = Booth::with(['event'])->where('event_id',$id)->get();
}else {
  $allbooths=[];
}
...
<div id="page-wrapper" style="min-height: 172px;">
			<div class="main-page">
                ...
                <thead>
                  ...
                  <tbody>
                    <?php $i=1;foreach ($allbooths as $key) {?>

                      <tr>
                        <td><?=$i;?></td>
                        <td><?=isset($key->event->eventName)?$key->event->eventName:'';?></td>
                        <td><?=$key->name;?></td>
                        <td><?=$key->price;?></td>
