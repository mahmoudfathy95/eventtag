...
$validate = new Dashboard();
    ...
    if($_SERVER['REQUEST_METHOD']=="POST")
    {
    	...
    	}elseif ($_POST['type']==3)
        {
            ...
            $delete_orders = Order::where('user_id',$id)->delete();
            
            $check_events = Event::where('user_id',$id)->get();
            foreach($check_events as $key)
            {
                $delete_booths = Booth::where('event_id',$key->id)->delete();
                
            }
            $delete_events = Event::where('user_id',$id)->delete();
