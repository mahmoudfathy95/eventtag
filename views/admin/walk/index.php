...
<div id="page-wrapper" style="min-height: 172px;">
			<div class="main-page">
				<div class="tables">
					...
					<h2 class="title1">walk through</h2>



					<div class="bs-example widget-shadow" data-example-id="contextual-table">

            <h4 style="display:inline-block">walk through :</h4>
    <?php
             $permission_create = Permission::where('name','walk through')->where('action','create')->first();
            if(check_permission($admin->group_id,$permission_create->id)){
             ?>
            <a class="btn btn-default" href="<?=$location;?>/views/admin/walk/create.php" style="background-color: #F2B33F;float: left;display: inline-block;">
...
<?php } ?>
                ...
                <thead>
                  ...
                  <tbody>
                    <?php $i=1; foreach ($allwalk as $key) {?>
                      <tr>
                        <td><?=$i;?></td>
                        <td><?=$key->title;?></td>
                        <td><img src="<?=$location;?>/<?=$key->img;?>" width="150"></td>
                        <td>
                            <?php
                           $permission_edit = Permission::where('name','walk through')->where('action','update')->first();
                          if(check_permission($admin->group_id,$permission_edit->id)){
                           ?>
                           <a href="<?=$location?>/views/admin/walk/edit.php?id=<?=$key->id;?>">
                        	<i class="fa fa-fw fa-edit"></i></td>
                           </a>
                           <?php }?>
                        <td>
                              <?php
                           $permission_delete = Permission::where('name','walk through')->where('action','delete')->first();
