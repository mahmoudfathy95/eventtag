...
$allorders = Order::with(['user','event','request'])->get();
?>
			...
			<div class="main-page">
                ...
                <thead>
                  <tr>
                    <th>#</th>
                    <th>رقم الطلب</th>
                    <th>اسم مسجل الطلب</th>
                    <th>اسم الايفنت</th>
                    <th>الاجمالى</th>
                    <th>الحالة</th>
                    <th>الاكشاك المحجوزة</th>
                    <th>حذف</th>
                  </tr>
                </thead>
                  <tbody>
                    <?php $i=1;foreach ($allorders as $key) {?>

                      <tr>
                        <td><?=$i;?></td>
                        <td><?=$key->id;?></td>
                        <td><?=isset($key->user->name)?$key->user->name:'';?></td>
                        <td><?=$key->event->eventName;?></td>
                        <th>
                            <?php
                            $total =0;
                            foreach ($key->request as $request)
                            {
                              $total +=$request->booths->price;
                        ...
                        <td class="activeAdmin">
                          <?php
                            ...
                            <div class="dic-active">
                              <!-- <button class="active" data-id="<?=$key->id;?>">
                                 <?php
                                   //echo $status;
                                  ?>
                              </button> -->
                        ...
                        <td>
                           <a href="<?=$location?>/views/admin/requests/?id=<?=$key->id;?>">
                          <i class="fa fa-fw fa-edit"></i>
                        </td>

                        <td>
                              <?php
                           $permission_delete = Permission::where('name','الطلبات')->where('action','delete')->first();
